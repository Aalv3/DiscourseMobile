...
Branch: `codex/notification-tap-routing`  
...
- the `adjusternetwork://auth_redirect` callback contract and its
  `isSafeAuthCallback` allowlist;
- the one-shot nonce/client-ID binding in `handleAuthPayload`;
...
`js/__tests__/authEphemeralSession.test.js` (14 tests):
...
- the callback stays restricted to `adjusternetwork://auth_redirect`;
...
3. `yarn ota:promote --group=<certified EAS group UUID>` — republishes that
   exact certified update group to `production`. Do not rebundle. The current
   production group is `39eb1e9b-8b72-480b-99f1-f52ad6d351fc`
   (iOS update `01a01ff3-d501-73b3-b799-2b4cf353efcb`, source
   `3fb9de379e499736513d6ade7227b5ce32201ba1`) and remains the rollback target.
