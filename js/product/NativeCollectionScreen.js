...
export default function NativeCollectionScreen({ navigation, route, screenProps }) {
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: colors.canvas }]}>
      ...
      {state.loading ? (
        <View style={styles.center}>
          ...
          <Text style={[styles.status, { color: colors.muted }]}>Loading {title}…</Text>
        </View>
      ) : state.error ? (
        <View style={styles.center}>
          <Text accessibilityRole="header" style={[styles.heading, { color: colors.text }]}>Content unavailable</Text>
          <Text style={[styles.status, { color: colors.muted }]}>This member collection could not be loaded.</Text>
          <Action label="Try again" secondary onPress={load} />
        </View>
      ) : (
        <ScrollView contentContainerStyle={styles.content}>
          <Text accessibilityRole="header" style={[styles.heading, { color: colors.text }]}>{title}</Text>
          <Text style={[styles.intro, { color: colors.muted }]}>Authenticated Adjuster Network content, presented natively.</Text>
          {state.topics.length ? state.topics.map(topic => (
            <Pressable
              accessibilityRole="link"
              key={topic.id}
              onPress={() => screenProps.openUrl(`${site.url}${topicPath(topic)}`)}
            ...
            <View style={[styles.empty, { backgroundColor: colors.surface, borderColor: colors.border }]}>
              <Text style={[styles.emptyTitle, { color: colors.text }]}>Nothing published yet</Text>
              <Text style={[styles.status, { color: colors.muted }]}>This collection is ready, but it does not have current member content.</Text>
            </View>
          )}
        </ScrollView>
      )}
...
const styles = StyleSheet.create({
  ...
  content: { width: '100%', maxWidth: 820, alignSelf: 'center', padding: spacing.md, paddingBottom: spacing.xl },
  heading: { fontSize: 26, lineHeight: 33, fontWeight: '800' },
  intro: { fontSize: 15, lineHeight: 22, marginTop: spacing.xs, marginBottom: spacing.md },
  status: { fontSize: 15, lineHeight: 22, textAlign: 'center' },
  topic: { minHeight: 76, borderWidth: StyleSheet.hairlineWidth, borderRadius: radius.md, padding: spacing.md, marginBottom: spacing.sm, justifyContent: 'center' },
  topicTitle: { fontSize: 16, lineHeight: 22, fontWeight: '700' },
  meta: { fontSize: 12, marginTop: spacing.xs },
  empty: { borderWidth: StyleSheet.hairlineWidth, borderRadius: radius.md, padding: spacing.lg, alignItems: 'center' },
